...
#plot using ggplot
library(ggplot2)
g1<-ggplot(df,aes(Date_Time, Global_active_power))+
  geom_line()+ylab("Global Active Power")+
  xlab("")+
...
g2<-ggplot(df,aes(Date_Time,Voltage))+geom_line()+xlab("datetime")+
  scale_x_datetime(breaks = as.POSIXct(c("2007-02-01 00:00:00",
                                         "2007-02-02 00:00:00",
                                         "2007-02-02 23:59:00"), 
                                       format = "%Y-%m-%d %H:%M:%OS"), 
                   labels = c("Thu","Fri","Sat"))
g3<-ggplot(df2,aes(Date_Time,value,color=variable))+geom_line()+
  ylab("Energy Sub Metering")+xlab("")+theme(legend.title = element_blank())+
  ...
  scale_x_datetime(breaks = as.POSIXct(c("2007-02-01 00:00:00",
                                       ...
                                       format = "%Y-%m-%d %H:%M:%OS"), 
                   labels = c("Thu","Fri","Sat"))
g4<-ggplot(df,aes(Date_Time, Global_reactive_power))+geom_line()+xlab("datetime")+
  scale_x_datetime(breaks = as.POSIXct(c("2007-02-01 00:00:00",
                                         "2007-02-02 00:00:00",
                                         "2007-02-02 23:59:00"), 
                                       format = "%Y-%m-%d %H:%M:%OS"), 
                   labels = c("Thu","Fri","Sat"))
#Arrange ggplots together
...
plot(df$Date_Time, df$Voltage, xlab="datetime",
     ylab="Voltage", type = "l")
